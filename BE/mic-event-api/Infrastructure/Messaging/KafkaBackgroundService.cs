            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                if (!string.IsNullOrEmpty(message))
                {
                    try
                    {
                        ...
                        if (jsonInterno != null)
                        {
                            Event? orderEvent = JsonSerializer.Deserialize<Event>(jsonInterno);
                            // 3. Crear scope para resolver el caso de uso Scoped
                            using var scope = _scopeFactory.CreateScope();
                            var useCase = scope.ServiceProvider.GetRequiredService<ICreateEventUseCase>();
                            // 4. Invocar el caso de uso
                            if (orderEvent != null)
                            {
                                await useCase.Execute(orderEvent);
                                Console.WriteLine($"[Kafka] Evento procesado y persistido: {orderEvent.Id}");
                            }
                            else
                            {
